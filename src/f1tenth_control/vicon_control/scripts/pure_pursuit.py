...
#Recieve traffic sign type from YOLO Model    
traffic_sign_detected = None  # global/shared state
...
class PurePursuit(object):
    ...
    def carstate_callback_gt(self, carstate_msg):
        self.x   = carstate_msg.pose.pose.position.x # meters
        self.y   = carstate_msg.pose.pose.position.y # meters
    ...
    def carstate_callback(self, carstate_msg):
        ...
        self.yaw = carstate_msg.data[2] # degrees
    ...
    def start_pp(self):
        ...
        while not rospy.is_shutdown():
            ...
            print(f"goal_arr: {goal_arr}")

            for val in goal_arr:
                print(f"Index: {val}, dist: {self.dist_arr[val]}, x: {self.path_points_x[val]}, y: {self.path_points_y[val]}")

            
            # finding the goal point which is the last in the set of points less than the lookahead distance
            for idx in reversed(goal_arr):
            ...
            if len(goal_arr) == 0:
                rospy.logwarn("No valid goal found. Skipping this iteration.")
                continue
            ...
            self.drive_msg.steering_angle = -f_delta #Direction of turining angle?
            self.ctrl_pub.publish(self.drive_msg)
            self.rate.sleep()    
